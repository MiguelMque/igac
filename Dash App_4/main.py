...
def build_radar(df,municipality):
    """
    This function generates the radar graphic that is shown in the app
    ...
    df : DataFrame
        ...
        and properti.
    municipality : String
        The municipality of interest to build the graphic

    Returns
    -------
    Scatterpolar graphic
        Graphic that shows the proportion of places in the municipality
        of interest.
...
def histogram_prices(df,municipality,end=600):
    """
    Function that builds a histogram with houses prices
    ...
    -------
    Histogram graphic
        Graphic that shows the distribution of the house prices in the
...
#Reading the data from postgress                       
df = pd.read_sql('data_for_model_clean_places',engine2)
#Dividing the house value by 1 million to make easier to read the value on graphics
...
dbc.Row([
    dbc.Button(
                            ...
                            id="go_button", className="button_instruction",style={'text-align':'center', 'margin-left': '45%'},#, color="info",
...
#Go the dashboar when we clic the "predict and describe" button on the main page        
@app.callback(dash.dependencies.Output('page-content', 'children'),
               [dash.dependencies.Input('url', 'pathname')])
 
def display_page(pathname):
     if pathname == '/appraisal&descriptive' :
         return Descriptivos()
     else: 
         return index
     
# #=== Descriptive tab, here we put together the graphics and 'about' text
@app.callback(
...
def individual_appraisal(rooms,baths,area,address,tipo,municipality,n_clicks):
    ...
    if n_clicks is None:
        ...
        try:
            lat,lon=get_coordenadas(address,municipality if municipality!="Fusagasugá" else "Fusagasuga",list(mostrar["Departamento"])[0])
            print(lat)
        except:
...
def bulk_appraisal(list_of_contents, list_of_names, list_of_dates):
    if list_of_contents is not None:
        ...
        for i in range(len(rooms)):
            try:
                lat,lon=get_coordenadas(address[i],municipality[i] if municipality[i]!="Fusagasugá" else "Fusagasuga",department[i])
            except:
